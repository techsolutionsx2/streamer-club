...
const CommentCard: React.FC<Props> = ({ data }) => {
    ...
    return (
        <>
            {!showEdit && (
                <Row
                    css={` background-color: ${themeGet("colors.gray.900")}; min-height: 120px; `}
                >
                    ...
                    >
                        ...
                        <SectionWrapper width="100%" >
                            ...
                            <SectionWrapper flexDirection="row" justifyContent="space-between" >
                                <Text
                                    fColor="red.100"
                                    fSize={0.75}
                                    fWeight={700}
                                    css={{ marginLeft: 13, cursor: "pointer" }}
                                    onClick={() => seekPlayerTimer(data?.video_time)}
                                >
                                    {`${moment.utc(data?.video_time * 1000).format('HH:mm:ss')}`}
                                </Text>

                                {isAdmin && <Text cursor="pointer" onClick={toggleEdit} >Edit</Text>}
                            ...
                            <Text
                                ...
                                css={{ marginTop: 5, marginLeft: 13 }}
                            >
                                {`${data?.players_detail?.user?.first_name ?? ''} ${data?.players_detail?.user?.last_name ?? ''} - ${data?.team?.club?.display_name}`}
                            </Text>
                        </SectionWrapper>
                    </Row>
                </Row>
